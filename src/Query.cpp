...
void Query::clear(void) { mysql_close(&conn); }
...
void Query::init(string host, string user, string passwd, string db,
                 unsigned int port) {
  mysql_init(&conn);
  res = NULL;
...
void Query::signupQuery(User *uptr) {
  ...
  sql.append("values( \"" + uptr->email + "\", \"" + uptr->first_name +
             "\", \"" + uptr->last_name + "\", \"" + uptr->passwd + "\", \"" +
             admin.c_str());
  sql.append("\");");
...
bool Query::loginQuery(User *uptr) {
  ...
  string sql = "";
  sql +=
      "select passwd, isadmin from tUser where userid=\"" + uptr->email + "\";";
  if (mysql_query(&conn, (char *)sql.c_str()))
    perror(__FILE__, __FUNCTION__, __LINE__);
  if (NULL == (res = mysql_store_result(&conn))) {
    perror(__FILE__, __FUNCTION__, __LINE__);
...
bool Query::sellItemQuery(User *uptr, Item *iptr, string *cur_time,
  ...
  sql.append("userid, date_bidending, cond, category, date_posted, "
             "description, unixtime_bidending, buyitnow_price) ");
  sql.append("values( \"" + uptr->email + "\" ," + " \"" +
             iptr->bid_ending_date + "\"" + ", " + " \"" + iptr->condition +
             "\", \"" + iptr->category + "\", \"" + *cur_time + "\" ");
  sql.append(",\"" + iptr->description + "\"" + +", \"" +
             to_string(unixtime_bidending) + "\", \"" + iptr->buyitnow_price +
             "\");");
...
void Query::showMyItemRows(User *uptr) {
  char buf[512] = {0};
  sprintf(buf,
          "select * from tItem natural join itemStatus natural left outer join "
          "maxBidder where userid = \"%s\";",
          uptr->email.c_str());
  string sql = buf;
  res = NULL;
  if (mysql_query(&conn, (char *)sql.c_str()))
  ...
  if (NULL == (res = mysql_store_result(&conn)))
    ...
    while ((row = mysql_fetch_row(res))) {
      ...
      cout << "  description: "
           << (row[DESCRIBE_FIELD] ? row[DESCRIBE_FIELD] : "") << endl;
      cout << "  status: "
           << (atoi(row[STATUS_FIELD]) == -2
                   ? "unsold"
                   : (atoi(row[STATUS_FIELD]) == -1
                          ? "sold"
                          : string(row[STATUS_FIELD]) + " bids"))
           << endl;
      cout << "  current bidding price: "
           << (row[PRICE_FIELD] ? row[PRICE_FIELD] : "0") << endl;
      cout << "  current highest bidder: "
           << (row[MAXBIDDER_FIELD] ? row[MAXBIDDER_FIELD] : "") << endl;
...
void Query::showItemRowsByDatePosted(string date) {
  ...
  sprintf(buf,
          "select * from tItem natural join itemStatus natural left outer join "
          "maxBidder where date_posted like \"%%%s%%\";",
          date.c_str());
  string sql = buf;
  res = NULL;
  if (mysql_query(&conn, (char *)sql.c_str()))
...
void Query::showItemRowsByDescriptionKeyword(string keyword) {
  ...
  sprintf(buf,
          "select * from tItem natural join itemStatus natural left outer join "
          "maxBidder where description like \"%%%s%%\";",
          keyword.c_str());
  string sql = buf;
  res = NULL;
  if (mysql_query(&conn, (char *)sql.c_str()))
...
void Query::checkYourAccount(User *uptr) {
  ...
  sprintf(buf_plus,
          "select itemid, description, price from soldItem where seller_userid "
          "= \"%s\";",
          uptr->email.c_str());
  sprintf(buf_minus,
          "select itemid, description, price from soldItem where buyer_userid "
          "= \"%s\";",
          uptr->email.c_str());
  sprintf(buf_balance,
          "select plus, comm, minus, balance from balanceSheet where userid = "
          "\"%s\";",
          uptr->email.c_str());
...
void Query::checkStatusOfYourBid(User *uptr) {
  char buf[512] = {0};
  sprintf(buf,
          "select itemid, description, bid_status, bid_price, price, "
          "date_bidending from bidStatus where buyer = \"%s\";",
          uptr->email.c_str());
  string sql = buf;
  res = NULL;
  if (mysql_query(&conn, (char *)sql.c_str()))
...
bool Query::buyItem(User &user, int itemid, Regex &re) {
  ...
  sprintf(buf, "select item_status from itemStatus where itemid = \"%s\";",
          itemid_str.c_str());
  string sql = buf;
  ...
  sprintf(buf, "call buyItNow(\"%s\",\"%s\",\"%s\",\"%s\");",
          user.email.c_str(), itemid_str.c_str(), cur_time_unix.c_str(),
          cur_time.c_str());
  sql = buf;
  if (mysql_query(&conn, (char *)sql.c_str())) {
    perror(__FILE__, __FUNCTION__, __LINE__);
...
bool Query::bidItem(User &user, int itemid, int bid_price, Regex &re) {
  ...
  sprintf(buf, "call postBid(\"%s\",\"%s\",\"%s\",\"%s\",\"%s\");",
          user.email.c_str(), itemid_str.c_str(), bid_price_str.c_str(),
          cur_time_unix.c_str(), cur_time.c_str());
  string sql = buf;
  if (mysql_query(&conn, (char *)sql.c_str())) {
    perror(__FILE__, __FUNCTION__, __LINE__);
    return false;
